...
const ChatLayout = () => {
    ...
    const handleSendMessage = async (text) => {
        ...
        try {
            const res = await geminiAPI.generate(text, selectedModel);
                ...
                <div className="model-selector-header">
                    ...
                    {user && (
                        <div className="user-info-header">
                            <span>Hi, {user.fullName?.split(' ')[0] || user.username}</span>
                        </div>
                    )}
                    {!user && anonymousMessageCount > 0 && (
                        <div className="guest-info">
                            <span>{ANONYMOUS_MESSAGE_LIMIT - anonymousMessageCount} free messages left</span>
                        </div>
                    )}
                </div>
